...
<!--EVENTS admin can edit/delete/update-->
<div>
        <?php

    $sql = "SELECT contentID, webTitle, webContents, userID, contents_dateAdded FROM Contents ORDER BY contentID DESC";
    ...
    if($result->num_rows > 0){
        ...
        while($row = $result->fetch_assoc()){
            ...
            echo "<h2>Title: " . $row["webTitle"]. "</h2>";
            echo "<h3>Contents: " . $row["webContents"]. "</h3>";
            echo "<h3>Author: " . $row["userID"]. "</h3>";
            echo "<h3>Date Added: " . $row["contents_dateAdded"]. "</h3>";
        ...
        echo "<form action='delete_contents.php' method='POST'>
    ...
    $current_time = date('Y-m-d H:i:s', time());
        echo "<form action='insert_content.php' method='post'>
                <input type='hidden' name='contentID' value='Anonymous'>
                <input type='hidden' name='contents_dateAdded' value='". $current_time ."'>
                <label for='webTitle'>Title</label><br>
                <input type='text' name='webTitle' placeholder='Title of event' required><br>
                <label for='webContents'>Contents:</label><br>
                <input type='text' name='webContents' placeholder='Description' required><br>
                <label for='userID'>User ID:</label><br>
                <input type='number' name='userID' placeholder='Put 1 for Admin' required><br>
                <br>
